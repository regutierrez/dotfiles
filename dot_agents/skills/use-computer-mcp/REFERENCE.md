...
```text
mcp({ tool: "computer_get_app_state", args: "{\"app\":\"company.thebrowser.Browser\",\"text_limit\":1000}" })
```
...
```text
mcp({ tool: "computer_get_app_state", args: "{\"app\":\"company.thebrowser.Browser\",\"max_tree_nodes\":3000,\"max_tree_depth\":96}" })
```
...
- `sky_click` uses the private macOS SkyLight background-window path. Use it when Chromium (including Arc) ignores `app_post` and the target window is current, on-screen, in the same Space, and possibly covered. It supports left single/double clicks and preserves foreground focus.
...
```text
mcp({ tool: "computer_click", args: "{\"app\":\"company.thebrowser.Browser\",\"x\":875,\"y\":375,\"click_method\":\"app_post\"}" })
```
...
- Space tags used with Finicky ATC: `akkio`, `horizon`
- Do not invent other Space names in this skill; extend `dot_config/finicky/finicky.js` and Arc ATC together if a new Space is added
- Some Arc chrome (Spaces switcher, Peek) can be sparse in the accessibility tree; prefer Finicky-routed URL opens over fighting the switcher UI when possible
