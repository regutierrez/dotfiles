...
For diff reviews, tie findings to behavior introduced, exposed, or made wrong by the change. For snapshot reviews, tie them to the requested path. Keep unrelated cleanup, style preferences, hypothetical consumers, and unsupported future-proofing out of the defect list.
...
Name the affected contract, the unresolved claim, and the test or inspection needed. Missing coverage does not prove a runtime defect. Keep verified findings even when other paths remain unresolved, but withhold a safety claim for those paths.

## Output

Lead with accepted findings, highest impact first. Give file and line evidence, trigger, consequence, owner, smallest fix, and proof. Follow [communication.md](communication.md) for explanations crossing modules, processes, lifecycle stages, or deployment steps.
