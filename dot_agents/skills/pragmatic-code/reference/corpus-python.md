...
Historical provenance retained from the Python review skill. The use/limit notes explain that skill's synthesis; current process and rule admission belong to [corpus.md](corpus.md) and the main skill. Source revisions below are evidence snapshots, not installed-version guarantees.
