...
Tailscale Serve (dev/staging/production):
  Queries connect to akkio-remote Tailscale Serve ports, not local Docker sidecars:
    dev 15432, staging 25432, production 35432.
  Gateway hostname: $HORIZON_PG_GATEWAY, else GATEWAY in
  ~/.config/akkio-vpn/client.conf, else akkio-remote.atlas-cherimoya.ts.net.
  local and ambient BACKEND_DB_URL / HORIZON_PG_URL (without --env) stay on the host route.
...
# Tailscale Serve TCP ports published on akkio-remote for Horizon VPN sidecars.
TAILSCALE_SERVE_PORTS = {"dev": 15432, "staging": 25432, "production": 35432}
DEFAULT_TAILSCALE_SERVE_GATEWAY = "akkio-remote.atlas-cherimoya.ts.net"
...
def tailscale_serve_gateway() -> str:
    """Return the Tailscale Serve gateway hostname for Horizon Postgres."""
    value = os.environ.get("HORIZON_PG_GATEWAY", "").strip()
    if value:
        return value
    config = Path(os.environ.get("XDG_CONFIG_HOME", str(Path.home() / ".config"))) / "akkio-vpn" / "client.conf"
    if config.is_file():
        for line in config.read_text().splitlines():
            ...
            if value:
                return value
            break
    return DEFAULT_TAILSCALE_SERVE_GATEWAY


def url_for_tailscale_serve(url: str, env: str) -> str:
    """Rewrite a Horizon Postgres URL onto the akkio-remote Tailscale Serve listener."""
    port = TAILSCALE_SERVE_PORTS.get(env)
    if not port:
        return url
    parsed = urllib.parse.urlparse(url)
    userinfo = parsed.netloc.rsplit("@", 1)[0] + "@" if "@" in parsed.netloc else ""
    gateway = tailscale_serve_gateway()
    return urllib.parse.urlunparse(parsed._replace(netloc=f"{userinfo}{gateway}:{port}"))
...
def reachable(url: str, env: str, timeout: float = 3.0) -> bool:
    route_url = url_for_tailscale_serve(url, env)
...
if mode == "envs":
    ...
    for env in ENVS:
        ...
        route_url = url_for_tailscale_serve(url, env)
        host, port = host_port(route_url)
        route = "tailscale" if env in TAILSCALE_SERVE_PORTS else "host"
...
url = url_for_tailscale_serve(url, route_env)
...
if os.environ.get("HORIZON_PG_SKIP_PREFLIGHT") != "1" and not reachable(url, route_env):
    die(
        ...
        "This environment's Tailscale Serve listener is not reachable. Run the 'envs' subcommand "
        "to see which environments are reachable, then retry with --env <name> or ask the user "
        "to check Tailscale / akkio-vpn doctor. "
        "Do not retry this same environment."
    )

route = "tailscale" if route_env in TAILSCALE_SERVE_PORTS else "host"
note(f"env={label} host={host}:{port} route={route} source={source.split(':', 1)[0]}")
