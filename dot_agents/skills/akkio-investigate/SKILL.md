...
4. **Build-layer — `~/blu-platform-transformations`.** When Snowflake contents are wrong, trace to the dbt model / YAML / lookback that built them, or a stale dbt Cloud run / supplemental-info cache. See `rca`'s `reference/data-layer.md` for mechanics.
