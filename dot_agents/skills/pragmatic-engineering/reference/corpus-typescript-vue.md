# TypeScript, Effect, and Vue Source Record

Historical provenance retained from the TypeScript/Vue review skill. The adopted/rejected lists record that skill's synthesis, not a second workflow. Current rule admission belongs to [corpus.md](corpus.md), while the active workflow owns execution.
...
- `installing-anti-slop-ts` owns plugin files, dependency discovery, Oxlint configuration, installation, migration, and rule rollout;
