...
When tracing recovery, inspect the child that emits the event as well as the parent handler. A parent clearing its own query or watcher does not end recovery if a rendered child owns independent polling and later emits another selection or completion event.
...
- Check contrast when styles change and reduced-motion behavior when animation changes.
