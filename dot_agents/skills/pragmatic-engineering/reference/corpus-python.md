# Python Source Record

This provenance supports the shared Python guidance; it does not define an execution workflow.

Historical provenance retained from the Python review skill. The use/limit notes explain that skill's synthesis; current rule admission belongs to [corpus.md](corpus.md), while the active workflow owns execution. Source revisions below are evidence snapshots, not installed-version guarantees.
