...
| Ownership | Policy, conversion, shared state, resources, and effects have an owner. If that design changes, apply [ownership.md](ownership.md) rather than judging by file length, wrapper count, or number of copies. Repeated code is not a defect when every copy enforces the same rule. |
