...
Pi keybindings come from `dot_pi/agent/keybindings.json` -> `~/.pi/agent/keybindings.json`. Current override keeps abort/interrupt on `escape` so double-escape session tree works, keeps clear/exit-on-second-press on `ctrl+c`, unbinds copy from `ctrl+c`, keeps selection cancel on `escape`, and frees `ctrl+p` / `shift+ctrl+p` by unbinding Pi's model-cycle/session-palette uses for a future command palette.
...
Pi extensions live under `dot_pi/agent/extensions/` -> `~/.pi/agent/extensions/`. `web-tools/` is vendored from `dmmulroy/.dotfiles` and provides `webfetch` plus Kagi-backed `websearch` (`KAGI_API_KEY` required); `run_onchange_after_30-install-pi-web-tools.sh.tmpl` runs `npm ci --omit=dev` for its runtime dependencies when package metadata changes. `dot_zshrc.tmpl` sources `~/.config/secrets/kagi.env` when present; keep that file untracked or age-encrypted.
