 ...
 * Replaces the `pi` token when a comment exists. Does not rename the Pi session file.
 */
 ...
 * `--display-agent` replaces the Agents-panel `agent` token (`pi`) with the first name row.
 * `$name2` is the wrap row and stays hidden until the comment overflows.
 */
export function buildHerdrDisplayAgentReportArgs(options: {
	...
	const rows = splitHerdrDisplayAgentRows(options.displayAgent);
	args.push(
		"--display-agent",
		rows.name1,
		"--clear-token",
