...
- `websearch` — search the public web with Kagi for current information and candidate URLs
...
- uses Kagi's hosted MCP server with `KAGI_API_KEY` from the environment
- calls the `kagi_search_fetch` MCP tool with Bearer authentication
- `depth` is accepted for tool compatibility but ignored by Kagi
...
- Kagi provider adapter: `home/.pi/agent/extensions/web-tools/providers/kagi.ts`
