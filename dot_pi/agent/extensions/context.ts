...
async function discoverExtensionFilesInDir(dir: string): Promise<string[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const isExtFile = /\.(ts|js)$/i.test(entry.name);

      if ((entry.isFile() || entry.isSymbolicLink()) && isExtFile) {
        out.push(entryPath);
        continue;
      }
...
async function discoverExtensionFiles(cwd: string): Promise<string[]> {
  const discovered = new Set<string>();
  for (const p of await discoverExtensionFilesInDir(path.join(cwd, ".pi", "extensions"))) {
    discovered.add(path.resolve(p));
  }
  for (const p of await discoverExtensionFilesInDir(path.join(getAgentDir(), "extensions"))) {
    discovered.add(path.resolve(p));
  }
  return [...discovered];
}
  ...
  pi.registerCommand("context", {
    ...
    handler: async (_args, ctx: ExtensionCommandContext) => {
      ...
      for (const t of pi.getAllTools()) {
        ...
        if (!p) continue;
        const arr = extensionsByPath.get(p) ?? [];
        if (!arr.includes(`[tool:${t.name}]`)) arr.push(`[tool:${t.name}]`);
        extensionsByPath.set(p, arr);
      }

      for (const p of await discoverExtensionFiles(ctx.cwd)) {
        const arr = extensionsByPath.get(p) ?? [];
        if (!arr.includes("[discovered]")) arr.push("[discovered]");
        extensionsByPath.set(p, arr);
      }
